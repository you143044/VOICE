...
def main():
    ...
    features_path = os.path.join('features', 'features_advanced.npy')
    labels_path = os.path.join('features', 'labels_advanced.npy')
    
    if not os.path.exists(features_path) or not os.path.exists(labels_path):
        print(f"\n错误: 特征文件不存在！")
        print(f"  特征文件: {features_path}")
        print(f"  标签文件: {labels_path}")
        print("请先运行 extract_features_light.py 提取特征！")
        return
    ...
    try:
        trainer.train(dataloaders, epochs=30)
        try:
